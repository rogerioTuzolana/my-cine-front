<template>
  <q-page class="q-pa-md">
    <div class="text-h5 q-pa-sm">Filmes</div>
    ...
    <q-table
      ...
      :rows="filmes"
      :columns="columns"
      :visible-columns="['nome', 'genero', 'duracao', 'options']"
      row-key="sala"
      :filter="filter"
      separator="cell"
    >
...
export default defineComponent({
  setup() {
    ...
    const listAll = async () => {
      try {
        loading.value = true;
        const auxFilme = await axios.get(`${url}cinema/admin/filmes`);

        filmes.value = auxFilme.data.data
        /*
        for (let index = 0; index < 10; index++) {
          filmes.value.push(formData.value)
        }
        */
        loading.value = false;
      } catch (error) {
